#!/usr/bin/env node
// Maths by Zosiama · video kit — master MP4 renderer.
//
...
//
//   node render.mjs <project> [ids…]                    MP4 masters → <project>/<slug>/<slug>.mp4
//   node render.mjs <project> --stills 0,5.5,40 q01     PNG stills  → <project>/build/<id>/still-<t>.png
//   node render.mjs <project> --thumbs [ids…]           poster.png (t = 0.5 s) per video
//
// <project> is a folder such as set-videos/ whose build/manifest.json lists the videos
// (written by the project's build.mjs through kit.mjs).
...
if (!args.length || args[0].startsWith('--')) { console.error('usage: node render.mjs <project> [--stills t,t,… | --thumbs] [ids…]'); process.exit(2); }
...
const todo = MANIFEST.filter((p) => !ids.length || ids.includes(p.id));
