#!/usr/bin/env python3
"""Maths by Zosiama · video kit — offline audio builder.
...
Usage: python3 audio.py <project>/build/script.json <project>/build [ids…]
"""
import hashlib
import json
import os
...
CACHE = os.environ.get('TTS_CACHE')  # default: <build dir>/tts-cache (set in main)
...
def speakable(text):
    ...
    # decimals digit by digit ("1.732." would otherwise be read "one. seven hundred…")
...
def tts(text):
    ...
    said = speakable(text)
    key = hashlib.sha1(f'{VOICE}|{SPEED}|{said}'.encode()).hexdigest()[:16]
    path = os.path.join(CACHE, key + '.wav')
    if not os.path.exists(path):
        samples, sr = kokoro().create(said, voice=VOICE, speed=SPEED, lang='en-us')
...
def main():
    ...
    only = set(sys.argv[3:])
    CACHE = CACHE or os.path.join(outroot, 'tts-cache')
